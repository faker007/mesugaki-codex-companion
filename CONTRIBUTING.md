...
- README는 생성 파일을 직접 수정하지 않고 `templates/README.ko.md.tmpl`과 `templates/README.ja.md.tmpl`을 수정한 뒤 `pnpm run readme`로 함께 생성한다.
- 설치 명령, 옵션, 경로, 환경 변수, 보안 경고를 바꿀 때는 두 언어의 대응 섹션을 함께 검토한다.
...
- `pnpm run readme:check`가 두 locale 모두 통과하는지, 일본어 문구를 원어민이 검수했는지 적는다.
