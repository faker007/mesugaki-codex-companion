...
스페인어·중국어 간체·중국어 번체 음성은 opener 설정의 `voice.languageAliases.es`, `voice.languageAliases.zh-Hans`, `voice.languageAliases.zh-Hant`를 검증된 다국어 alias에 연결한 뒤 wrapper에 canonical `--language` 값을 전달해. scripted setup에서는 `--language-alias=zh-Hans:<voice-alias>` 같은 옵션을 반복해서 지정할 수 있어. 명시적인 `--voice`가 있으면 그 값을 우선하고, mapping이 없으면 기본 한국어 voice로 몰래 fallback하지 않고 provider 호출 전에 실패해. `zh-Hant`는 번체 문자를 뜻하며 광둥어를 의미하지 않아.
...
README는 locale별 template에서 함께 생성돼. **생성된 README를 직접 고치지 말고** `templates/README.*.md.tmpl` 5개를 함께 수정해.
